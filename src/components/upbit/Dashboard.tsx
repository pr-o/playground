...
export function Dashboard() {
  ...
  const { asks, bids, loading: orderbookLoading, error: orderbookError } = useOrderbook(market);
  const { trades, loading: tradesLoading, error: tradesError } = useTrades(market);
  return (
    <div className="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="flex flex-col gap-2">
          <h1 className="text-2xl font-semibold text-foreground sm:text-3xl">Upbit Market Overview</h1>
